...
def test_nagios(url, username, password):
    try:
        s = requests.session()
...
def login_nagios(url, username, password):
    s = requests.session()
    auth = username + ':' + password
    auth = base64.b64encode(auth)
    myheader = {
    ...
    # get services data
    service_url = scheme + '://' + root +'/nagios/cgi-bin/status.cgi?host=all&servicestatustypes=28&limit=0'
    ...
    # get hosts data
    host_url = scheme + '://' + root + '/nagios/cgi-bin/status.cgi?hostgroup=all&style=hostdetail&hoststatustypes=12&limit=0'
...
if __name__ == '__main__':
    ...
    username = 'admin'
    password = '*****'
    print login_nagios(url=url, username=username, password=password)
    pass
